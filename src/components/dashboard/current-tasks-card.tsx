...
import { format, formatDistanceToNow, parseISO, isValid, differenceInHours } from "date-fns";
                ...
                {pinnedTasks.length > 0 ? (
                    <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                        {pinnedTasks.map(task => {
                            ...
                            const duration = task.startDate && task.deadline ? differenceInHours(parseISO(task.deadline), parseISO(task.startDate)) : null;
                            return (
                                <div key={task.id} className={cn("p-4 rounded-lg bg-background/50 border border-border flex flex-col gap-3 relative group", task.completed && "opacity-50")}>
                                   ...
                                   <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mt-auto pt-3 border-t border-border">
                                        ...
                                        <div>
                                            ...
                                            <p>{duration !== null ? `${duration} hr(s)` : 'N/A'}</p>
                                        </div>
                                        <div>
